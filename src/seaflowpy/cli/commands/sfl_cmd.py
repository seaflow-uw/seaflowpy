...
def sfl_db_cmd(infile, db, cruise, force, json, serial, verbose):
    ...
    if cruise is None or serial is None:
        raise click.ClickException('instrument serial and cruise must both be specified either in filename as <cruise>_<instrument-serial>.sfl or as command-line options.')
    ...
    if len(errors) > 0:
        ...
        if not force:
            sys.exit(1)
    sfl.save_to_db(df, db, cruise, serial)
